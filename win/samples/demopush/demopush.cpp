...
public:
	...
	int OnCreate(LPCREATESTRUCT /*lpCreateStruct*/)
	{
		...
		unsigned cnt = Ogmacam_EnumV2(m_dev);
		if (0 == cnt)
	...
	void OnDataCallbackV4(const void* pData, const OgmacamFrameInfoV3* pInfo, int bSnap)
	{
		if (NULL == pData)
			PostMessage(MSG_CAMEVENT, OGMACAM_EVENT_ERROR);
		else if (bSnap)
		{
			...
			header.biWidth = pInfo->width;
			header.biHeight = pInfo->height;
			...
			if (pSnapData)
			{
				memcpy(pSnapData, pData, TDIBWIDTHBYTES(header.biWidth * header.biBitCount) * header.biHeight);
		...
		{
			if ((pInfo->width != m_header.biWidth) || (pInfo->height != m_header.biHeight))
				return;
			{
				std::lock_guard<std::mutex> lock(m_mutex);
				memcpy(m_pData, pData, TDIBWIDTHBYTES(m_header.biWidth * m_header.biBitCount) * m_header.biHeight);
			}
			PostMessage(MSG_CAMIMAGE);
		}
	}

	static void __stdcall StaticOnDataCallbackV4(const void* pData, const OgmacamFrameInfoV3* pInfo, int bSnap, void* pCallbackCtx)
	{
		CMainFrame* pThis = (CMainFrame*)pCallbackCtx;
		pThis->OnDataCallbackV4(pData, pInfo, bSnap);
	}
			...
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Ogmacam_Stop(m_hcam)))
				{
					...
					if (SUCCEEDED(Ogmacam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						...
						Ogmacam_StartPushModeV4(m_hcam, StaticOnDataCallbackV4, this, StaticOnEventCallback, this);
					}
				}
			}
		}
	...
	{
		CloseDevice();
		AtlMessageBox(m_hWnd, _T("Camera disconnect."));
	}
