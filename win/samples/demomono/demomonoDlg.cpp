...
BEGIN_MESSAGE_MAP(CdemomonoDlg, CDialog)
	...
	ON_WM_HSCROLL()
	ON_COMMAND_RANGE(IDM_SNAP_RESOLUTION, IDM_SNAP_RESOLUTION + OGMACAM_MAX, &CdemomonoDlg::OnSnapResolution)
END_MESSAGE_MAP()
...
void CdemomonoDlg::OnBnClickedButton1()
{
	...
	m_maxBitDepth = Ogmacam_get_MaxBitDepth(m_hcam);

	if ((8 == m_maxBitDepth) || IsDlgButtonChecked(IDC_CHECK2))
	{
		m_bBitDepth = false;
		CheckDlgButton(IDC_CHECK2, 0);
		Ogmacam_put_Option(m_hcam, OGMACAM_OPTION_RGB, 3);
		Ogmacam_put_Option(m_hcam, OGMACAM_OPTION_BITDEPTH, 0);
	}
	else
	{
		m_bBitDepth = true;
		Ogmacam_put_Option(m_hcam, OGMACAM_OPTION_RGB, 4);
		Ogmacam_put_Option(m_hcam, OGMACAM_OPTION_BITDEPTH, 1);
	}
