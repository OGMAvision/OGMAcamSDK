...
private:
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Ogmacam_put_ExpoTime(m_hcam, nTime);
		EndDialog(wID);
		return 0;
	}
...
private:
			...
			if (val == 2)
			{
				...
				if (E_INVALIDARG == hr)
				{
					if (m_nTriggerNumber > 1)
						AtlMessageBox(m_hWnd, L"OGMACAM_FLAG_TRIGGER_SINGLE: only number = 1 supported");
				}
			}
		}
	}
	...
	void UpdateFrameText()
	{
		...
		if (m_hcam && SUCCEEDED(Ogmacam_get_FrameRate(m_hcam, &nFrame, &nTime, &nTotalFrame)))
		{
			wchar_t str[256];
			if (nTime >= 1000)
	...
	void UpdateFrameInfoText(const OgmacamFrameInfoV4& info)
	{
		wchar_t str[256];
