...
{
	int bAutoExp = 0;
	Ogmacam_get_AutoExpoEnable(g_hcam, &bAutoExp);
...
{
	Ogmacam_put_AutoExpoEnable(g_hcam, ((CButton*)GetDlgItem(IDC_CHECK_AUTO))->GetCheck() ? 1 : 0);
	UpdateSlidersEnable();
}
...
void CExposureGainPropertyPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (pScrollBar == GetDlgItem(IDC_SLIDER_TARGET))
	{
		...
		Ogmacam_get_AutoExpoTarget(g_hcam, &curTarget);
		unsigned short target = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TARGET))->GetPos();
		if (target != curTarget)
		{
			Ogmacam_put_AutoExpoTarget(g_hcam, target);
	...
	{
		unsigned curTime = 0;
		Ogmacam_get_ExpoTime(g_hcam, &curTime);
		unsigned time = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_EXPOSURE))->GetPos();
		if (time != curTime)
		{
			Ogmacam_put_ExpoTime(g_hcam, time);
			CString str;
	...
	else if (pScrollBar == GetDlgItem(IDC_SLIDER_GAIN))
	{
		...
		Ogmacam_get_ExpoAGain(g_hcam, &curGain);
		unsigned short gain = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_GAIN))->GetPos();
		if (gain != curGain)
		{
			Ogmacam_put_ExpoAGain(g_hcam, gain);
