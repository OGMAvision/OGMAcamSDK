...
void CPreviewSnapPropertyPage::UpdateSnapRes()
{
	if (g_hcam)
	{
		const int resCnt = Ogmacam_get_StillResolutionNumber(g_hcam);
		...
		else
		{
			...
			{
				Ogmacam_get_StillResolution(g_hcam, i, &width, &height);
				CString str;
				str.Format(_T("%d x %d"), width, height);
				resList->AddString(str);
			}
...
BOOL CPreviewSnapPropertyPage::OnInitDialog()
{
	...
	if (g_hcam)
	{
		const int resCnt = Ogmacam_get_ResolutionNumber(g_hcam);
		CComboBox* previewResList = (CComboBox*)GetDlgItem(IDC_COMBO_PREVIEW);
		int width = 0, height = 0;
		for (int i = 0; i < resCnt; ++i)
		{
			Ogmacam_get_Resolution(g_hcam, i, &width, &height);
			CString str;
			str.Format(_T("%d x %d"), width, height);
			previewResList->AddString(str);
		}
		unsigned resIndex = 0;
		Ogmacam_get_eSize(g_hcam, &resIndex);
...
void CPreviewSnapPropertyPage::OnBnClickedButtonSnap()
{
	const int resCnt = Ogmacam_get_StillResolutionNumber(g_hcam);
	if (resCnt <= 0)
		Ogmacam_Snap(g_hcam, 0xffffffff);
	else
	{
		CComboBox* resList = (CComboBox*)GetDlgItem(IDC_COMBO_SNAP);
		Ogmacam_Snap(g_hcam, resList->GetCurSel());
	}
}
