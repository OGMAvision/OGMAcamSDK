...
private:
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		const unsigned num = Ogmacam_EnumV2(arr);
		if (num <= 0)
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		else
		{
			...
			Ogmacam_get_ExpTimeRange(m_hcam, &minTime, &maxTime, &defTime);
			...
			Ogmacam_put_ExpoTime(m_hcam, expoTime);
			if (FAILED(Ogmacam_StartPullModeWithCallback(m_hcam, StaticCameraCallback, this)))
			{
...
private:
	...
	void OpenCamera(const wchar_t* camId, const OgmacamModelV2* model)
	{
		...
		m_model = model;
		if (0 == (m_model->flag & OGMACAM_FLAG_TRIGGER_SOFTWARE))
		{
