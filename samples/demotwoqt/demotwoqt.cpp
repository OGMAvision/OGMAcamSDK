    ...
    : QWidget(parent), m_timer(new QTimer(this))
{
    ...
    QHBoxLayout* hlayout = new QHBoxLayout(this);
    {
        QVBoxLayout* vlayout = new QVBoxLayout();
        {
            ...
            {
                if (m_hcam[0])
                    Ogmacam_put_AutoExpoEnable(m_hcam[0], state ? 1 : 0);
                if (m_hcam[1])
    ...
    connect(m_timer, &QTimer::timeout, this, [this]()
    {
        ...
        if (m_hcam[0] && SUCCEEDED(Ogmacam_get_FrameRate(m_hcam[0], &nFrame, &nTime, &nTotalFrame)) && (nTime > 0))
            m_lbl_frame[0]->setText(QString::asprintf("%u, fps = %.1f", nTotalFrame, nFrame * 1000.0 / nTime));
        if (m_hcam[1] && SUCCEEDED(Ogmacam_get_FrameRate(m_hcam[1], &nFrame, &nTime, &nTotalFrame)) && (nTime > 0))
...
{
    m_hcam[idx] = Ogmacam_Open(camId);
    if (m_hcam[idx])
    {
        Ogmacam_get_Size(m_hcam[idx], &m_imgWidth[idx], &m_imgHeight[idx]);
        Ogmacam_put_Option(m_hcam[idx], OGMACAM_OPTION_BYTEORDER, 0); //Qimage use RGB byte order
        Ogmacam_put_AutoExpoEnable(m_hcam[idx], m_cbox_auto->isChecked() ? 1 : 0);

        m_pData[idx] = new uchar[TDIBWIDTHBYTES(m_imgWidth[idx] * 24) * m_imgHeight[idx]];
        m_ctx[idx].pthis = this;
        m_ctx[idx].idx = idx;
        if (FAILED(Ogmacam_StartPullModeWithCallback(m_hcam[idx], eventCallBack, &m_ctx[idx])))
        {
            closeCamera(idx);
...
void MainWidget::onBtnOpen()
{
    ...
    const unsigned count = Ogmacam_EnumV2(arr);
    if (0 == count)
...
void MainWidget::handleImageEvent(int idx)
{
    ...
    if (SUCCEEDED(Ogmacam_PullImage(m_hcam[idx], m_pData[idx], 24, &width, &height)))
    {
