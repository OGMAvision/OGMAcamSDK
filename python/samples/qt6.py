...
class MainWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.pData = None
        self.res = 0
        self.temp = ogmacam.OGMACAM_TEMP_DEF
        self.tint = ogmacam.OGMACAM_TINT_DEF
        self.count = 0
        ...
        self.slider_expoTime = QSlider(Qt.Orientation.Horizontal)
        self.slider_expoGain = QSlider(Qt.Orientation.Horizontal)
        ...
        self.lbl_temp = QLabel(str(ogmacam.OGMACAM_TEMP_DEF))
        self.lbl_tint = QLabel(str(ogmacam.OGMACAM_TINT_DEF))
        self.slider_temp = QSlider(Qt.Orientation.Horizontal)
        self.slider_tint = QSlider(Qt.Orientation.Horizontal)
        self.slider_temp.setRange(ogmacam.OGMACAM_TEMP_MIN, ogmacam.OGMACAM_TEMP_MAX)
        self.slider_temp.setValue(ogmacam.OGMACAM_TEMP_DEF)
        self.slider_tint.setRange(ogmacam.OGMACAM_TINT_MIN, ogmacam.OGMACAM_TINT_MAX)
        self.slider_tint.setValue(ogmacam.OGMACAM_TINT_DEF)
    ...
    def startCamera(self):
        self.pData = bytes(ogmacam.TDIBWIDTHBYTES(self.imgWidth * 24) * self.imgHeight)
        ...
        if self.cur.model.flag & ogmacam.OGMACAM_FLAG_MONO == 0:
        ...
        try:
            ...
            self.btn_autoWB.setEnabled(self.cur.model.flag & ogmacam.OGMACAM_FLAG_MONO == 0)
            self.slider_temp.setEnabled(self.cur.model.flag & ogmacam.OGMACAM_FLAG_MONO == 0)
            self.slider_tint.setEnabled(self.cur.model.flag & ogmacam.OGMACAM_FLAG_MONO == 0)
    ...
    def openCamera(self):
        self.hcam = ogmacam.Ogmacam.Open(self.cur.id)
        if self.hcam:
        ...
        if self.hcam:
            if 0 == self.cur.model.still:    # not support still image capture
                if self.pData is not None:
                    image = QImage(self.pData, self.imgWidth, self.imgHeight, ogmacam.TDIBWIDTHBYTES(self.imgWidth * 24), QImage.Format.Format_RGB888)
                    self.count += 1
    ...
    def eventCallBack(nEvent, self):
        '''callbacks come from ogmacam.dll/so internal threads, so we use qt signal to post this event to the UI thread'''
        self.evtCallback.emit(nEvent)

    def onevtCallback(self, nEvent):
        '''this run in the UI thread'''
        if self.hcam:
            if ogmacam.OGMACAM_EVENT_IMAGE == nEvent:
                self.handleImageEvent()
            elif ogmacam.OGMACAM_EVENT_EXPOSURE == nEvent:
                self.handleExpoEvent()
            elif ogmacam.OGMACAM_EVENT_TEMPTINT == nEvent:
                self.handleTempTintEvent()
            elif ogmacam.OGMACAM_EVENT_STILLIMAGE == nEvent:
                self.handleStillImageEvent()
            elif ogmacam.OGMACAM_EVENT_ERROR == nEvent:
                self.closeCamera()
    ...
    def handleImageEvent(self):
        try:
            ...
            image = QImage(self.pData, self.imgWidth, self.imgHeight, ogmacam.TDIBWIDTHBYTES(self.imgWidth * 24), QImage.Format.Format_RGB888)
            newimage = image.scaled(self.lbl_video.width(), self.lbl_video.height(), Qt.AspectRatioMode.KeepAspectRatio)
    ...
    def handleStillImageEvent(self):
        info = ogmacam.OgmacamFrameInfoV3()
        try:
            ...
            if info.width > 0 and info.height > 0:
                buf = bytes(ogmacam.TDIBWIDTHBYTES(info.width * 24) * info.height)
                try:
                    self.hcam.PullImageV3(buf, 1, 24, 0, info)
                except ogmacam.HRESULTException:
                    pass
                else:
                    image = QImage(buf, info.width, info.height, ogmacam.TDIBWIDTHBYTES(info.width * 24), QImage.Format.Format_RGB888)
                    self.count += 1
                    image.save("pyqt{}.jpg".format(self.count))

if __name__ == '__main__':
    ogmacam.Ogmacam.GigeEnable(None, None)
