...
std::vector<TDshowDevice> dscap_enum_device()
{
	...
	if (g_pDshowEnumCamera)
	{
		...
		{
			TCHAR str[256];
			_stprintf(str, _T("Ogmacam #%u"), i); /* Ogmacam #0, Ogmacam #1, etc */
...
HRESULT TDshowContext::InitCapFilter()
{
	...
	{
		unsigned nIndex = 0;
		const wchar_t* p = _tcschr(m_devname.c_str(), '#'); // extract the index from the name, such as 'Ogmacam #0', 'Ogmacam #1'
		if (p)
			nIndex = _ttoi(p + 1);
