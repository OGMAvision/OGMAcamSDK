...
void MainWidget::onBtnOpen()
{
    ...
    {
		m_hcam = Ogmacam_Open(arr[0].id);
		if (m_hcam)
		{
			Ogmacam_get_Size(m_hcam, &m_imgWidth, &m_imgHeight);
			Ogmacam_put_Option(m_hcam, OGMACAM_OPTION_BYTEORDER, 0); //Qimage use RGB byte order
            Ogmacam_put_AutoExpoEnable(m_hcam, m_cbox_auto->isChecked() ? 1 : 0);
            ...
            if (FAILED(Ogmacam_StartPullModeWithCallback(m_hcam, CameraCallBack, this)))
			{
				closeCamera();
